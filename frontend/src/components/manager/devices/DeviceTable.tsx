...
interface DeviceTableProps {
  ...
  onDelete: (id: string) => void; // Hàm xóa thiết bị
          ...
          <tbody className="divide-y divide-slate-100">
            {devices.map((device) => (
              <tr key={device.id} className="hover:bg-slate-50">
                <td className="px-6 py-4 text-sm font-bold text-slate-800">{device.id}</td>

                <td className="px-6 py-4 text-sm">
                  <p className="font-semibold text-slate-800">{device.name}</p>

                  <p className="text-xs text-slate-400 mt-0.5">
                    Ngày nhập: {device.importDate}
                  </p>
                ...
                <td className="px-6 py-4 text-sm font-semibold">{device.quantity}</td>
